...
object SparkScalaBitcoinTransactionGraph {
	...
	def jobTop5AddressInput(sc: SparkContext, hadoopConf: Configuration, inputFile: String, outputFile: String,back_type: String,central_addreess:String): Unit = {
		...
		var centralTranscations=btcDF.filter($"dest_address".equalTo("bitcoinaddress_"+central_addreess))
		val sameTranscation=centralTranscations.join(btcDF,centralTranscations("curr_trans_hash")===btcDF("curr_trans_hash"))
		...
		val joined_degree1=sameTranscation.join(sourceDF,sameTranscation("curr_trans_input_hash")===sourceDF("source_trans_hash")&&centralTranscations("curr_trans_input_output_idx")===sourceDF("source_trans_output_idx"))
		joined_degree1.show(10)
		joined_degree1.select($"dest_address",$"value",$"source_address",$"source_value",$"timestamp").withColumn("curr_trans_hash", lit($"curr_trans_hash").cast("string")).distinct.write.format("com.databricks.spark.csv").option("header", "true").save(outputFile+"/"+back_type+"/degree1.csv")
		...
		val joined_degree2=same_degree1.join(sourceDF,same_degree1("curr_trans_input_hash")===sourceDF("source_trans_hash")&&source_degree1("curr_trans_input_output_idx")===sourceDF("source_trans_output_idx"))
		joined_degree2.show(10)
		joined_degree2.select($"dest_address",$"value",$"source_address",$"source_value",$"timestamp").withColumn("curr_trans_hash", lit($"curr_trans_hash").cast("string")).distinct.write.format("com.databricks.spark.csv").option("header", "true").save(outputFile+"/"+back_type+"/degree2.csv")


		val source_degree2=joined_degree2.select($"source_address".alias("dest_address"), $"source_trans_input_hash".alias("curr_trans_input_hash"),$"source_trans_input_output_idx".alias("curr_trans_input_output_idx"),$"source_trans_hash".alias("curr_trans_hash"), $"source_trans_output_idx".alias("curr_trans_output_idx"),$"source_timestamp".alias("timestamp"),$"source_value".alias("value"))
		val same_degree2=source_degree2.join(btcDF,source_degree2("curr_trans_hash")===btcDF("curr_trans_hash"))
		val joined_degree3=same_degree2.join(sourceDF,same_degree2("curr_trans_input_hash")===sourceDF("source_trans_hash")&&source_degree2("curr_trans_input_output_idx")===sourceDF("source_trans_output_idx"))
		joined_degree2.show(10)
		joined_degree2.select($"dest_address",$"value",$"source_address",$"source_value",$"timestamp").withColumn("curr_trans_hash", lit($"curr_trans_hash").cast("string")).distinct.write.format("com.databricks.spark.csv").option("header", "true").save(outputFile+"/"+back_type+"/degree3.csv")

//		val source_degree3=joined_degree3.select($"source_address".alias("dest_address"), $"source_trans_input_hash".alias("curr_trans_input_hash"),$"source_trans_input_output_idx".alias("curr_trans_input_output_idx"),$"source_trans_hash".alias("curr_trans_hash"), $"source_trans_output_idx".alias("curr_trans_output_idx"),$"source_timestamp".alias("timestamp"),$"source_value".alias("value"))
//		val joined_degree4=source_degree3.join(sourceDF,source_degree3("curr_trans_input_hash")===sourceDF("source_trans_hash")&&source_degree3("curr_trans_input_output_idx")===sourceDF("source_trans_output_idx"))
//		joined_degree2.show(10)
//		joined_degree2.select($"dest_address",$"value",$"source_address",$"source_value",$"timestamp",$"curr_trans_hash").distinct.write.format("com.databricks.spark.csv").option("header", "true").save(outputFile+"/"+back_type+"/degree4.csv")
	}
